...
namespace desktop {
    ...
    void ExchangeTableModel::updateTransactionList(QList<ExchangeTransaction> list)
    {
        beginResetModel();
    ...
    QVariant ExchangeTableModel::data(const QModelIndex &index, int role) const
    {
        ...
        switch (index.column()) {
        case 0:
            return QVariant::fromValue(transactionList.at(index.row()).getCurrency()).toString().toUpper();
        case 1:
